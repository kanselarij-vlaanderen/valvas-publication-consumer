  ...
  async download() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        throw new Error(`Request to download file returned status code ${result.status} (download url: ${this.downloadUrl})`);
      }
    } catch (e) {
...
async function syncDocuments(distributions) {
  const documents =  distributions.filter(d => d.attributes['type'] === DISTRIBUTION_ATTACHMENT_TYPE && d.attributes['download-url'])
